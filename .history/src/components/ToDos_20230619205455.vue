...
const _todo_list: ComputedRef<Array<ToDo>> = computed(() => []);
const _pending = computed(() => {
  return _todo_list.value;
});
...
<template>
  <div class="todo-container w3-white w3-card-4">
    ...
    <div class="w3-padding" v-for="todo in _pending.values" :key="todo.id">
      <label>
    ...
    <div class="w3-padding" v-for="todo in _done.values" :key="todo.id">
      <label>
        <input type="checkbox" v-model="todo.checked" />
        <span class="w3-margin-left">{{ todo.text }}</span>
      </label>
